...
export interface ClientToServerEvents {
  "room:create": (
    ...
    ack: (response: AckResponse<CreateRoomOutput>) => void
  ) => void;
  "room:join": (
    payload: JoinRoomInput,
    ack: (response: AckResponse<RoomPublicState>) => void
  ) => void;
  "room:start": (
    payload: StartGameInput,
    ack: (response: AckResponse<undefined>) => void
  ) => void;
  "room:leave": (
  ...
  "room:updateOptions": (
    payload: UpdateRoomOptionsInput,
    ack: (response: AckResponse<undefined>) => void
  ) => void;
  "game:submitVote": (
  ...
  "game:submitStatement": (
    payload: SubmitStatementInput,
    ack: (response: AckResponse<undefined>) => void
  ) => void;
  "room:setRematchReady": (
    payload: SetRematchReadyInput,
    ack: (response: AckResponse<undefined>) => void
  ) => void;
  "room:startRematch": (
    payload: StartRematchInput,
    ack: (response: AckResponse<undefined>) => void
  ) => void;
}
...
export interface ServerToClientEvents {
  ...
  "game:started": (payload: { state: RoomPublicState; self: PlayerSecret }) => void;

...
const nextSpeakDeadline = () => Date.now() + SPEAK_TURN_MS;
const getUndercoverCount = (playerCount) => (playerCount >= 6 ? 2 : 1);
const getBlankCount = (playerCount, blankRoleEnabled) => blankRoleEnabled && playerCount >= 5 ? 1 : 0;
...
export const startGame = (room) => {
    ...
    room.players.forEach((player) => {
        ...
        const role = undercoverIds.has(player.id)
            ? "undercover"
...
export const endCurrentSpeakerTurn = (room) => {
    if (!room.gameState) {
...
export const submitStatement = (room, playerId, rawText) => {
    if (!room.gameState) {
        throw new Error("游戏尚未开始");
    }
    const game = room.gameState;

...
export class RoomManager {
  ...
  private socketToRoomCode = new Map<string, string>();
  ...
  createRoom(socketId: string, nickname: string, clientId = socketId): RoomPublicState {
  ...
  joinRoom(socketId: string, roomCodeRaw: string, nickname: string, clientId = socketId): RoomPublicState {
  ...
  getPublicState(roomCode: string): RoomPublicState {
    const room = this.rooms.get(roomCode);
    if (!room) {
      throw new Error("房间不存在");
    }
  ...
  getPlayerSecret(socketId: string): PlayerSecret {
  ...
  startGameBySocket(socketId: string): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
  ...
  nextPhaseBySocket(socketId: string): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    if (!playerId || room.hostId !== playerId) {
  ...
  endTurnBySocket(socketId: string): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    if (!playerId) {
  ...
  voteBySocket(socketId: string, targetPlayerId: string): {
  ...
  submitStatementBySocket(socketId: string, text: string): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    if (!playerId) {
      throw new Error("玩家不存在");
  ...
  leaveBySocket(socketId: string): RoomPublicState | null {
  ...
  disconnectBySocket(socketId: string): RoomPublicState | null {
    const roomCode = this.socketToRoomCode.get(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    this.socketToRoomCode.delete(socketId);
    this.socketToPlayerId.delete(socketId);
  ...
  setRematchReadyBySocket(socketId: string, ready: boolean): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
  ...
  startRematchBySocket(socketId: string): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    if (!playerId || room.hostId !== playerId) {
  ...
  updateRoomOptionsBySocket(socketId: string, blankRoleEnabled: boolean): RoomPublicState {
    const room = this.getRoomBySocket(socketId);
    const playerId = this.socketToPlayerId.get(socketId);
    if (!playerId || room.hostId !== playerId) {
  ...
  tickSpeakTimeouts(now = Date.now()): RoomPublicState[] {
    const changedStates: RoomPublicState[] = [];
  ...
  tickCleanupRooms(now = Date.now()): void {
    for (const room of this.rooms.values()) {
      if (room.players.length > 0) {
        room.emptySince = null;
        continue;
  ...
  private applyWinnerStats(room: InternalRoom, winner: PlayerRole): void {
    const game = room.gameState;
    if (!game) {
      return;
    }
  ...
  private getRoomBySocket(socketId: string): InternalRoom {
    const roomCode = this.socketToRoomCode.get(socketId);
    if (!roomCode) {
